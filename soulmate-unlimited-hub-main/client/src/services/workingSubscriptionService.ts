...
export class WorkingSubscriptionService {
  ...
  async getUserUsage(userId: string): Promise<UserUsage | null> {
    try {
      ...
      if (Array.isArray(data) && data.length > 0) {
        const usage = data[0] as unknown as UserUsage;
        return {
          daily_likes_used: usage.daily_likes_used || 0,
          daily_super_likes_used: usage.daily_super_likes_used || 0,
          daily_boosts_used: usage.daily_boosts_used || 0,
          last_like_reset_date: usage.last_like_reset_date || '',
          role: usage.role || 'registered'
        };
      } else if (typeof data === 'object' && !Array.isArray(data)) {
        const usage = data as unknown as UserUsage;
        return {
          daily_likes_used: usage.daily_likes_used || 0,
          daily_super_likes_used: usage.daily_super_likes_used || 0,
          daily_boosts_used: usage.daily_boosts_used || 0,
  ...
  async resetDailyUsage(userId: string): Promise<boolean> {
    try {
      const { data, error } = await supabase.rpc('reset_daily_usage');
...
export const workingSubscriptionService = new WorkingSubscriptionService();