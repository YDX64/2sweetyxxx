    ...
    if (likes.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <h3 className="text-xl font-semibold mb-2">{t('likes.noLikesYet')}</h3>
          <p className="text-gray-600">{t('likes.startLiking')}</p>
        </div>
      );
    }
          ...
          <Card key={profile.id} className="overflow-hidden hover:shadow-lg transition-shadow">
            ...
            <CardContent className="p-4">
              ...
              <div className="flex items-center text-xs text-gray-500">
                ...
                {t('likes.likedOn')} {new Date(profile.created_at).toLocaleDateString('tr-TR')}
              </div>
            </CardContent>
          </Card>
        ))}
    ...
    <div className="min-h-screen bg-gradient-to-br from-pink-50 via-red-50 to-orange-50 dark:from-gray-900 dark:via-gray-800 dark:to-gray-900">
      ...
      <main className="container mx-auto px-4 py-8">
        <div className="max-w-6xl mx-auto">
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-gray-900 mb-2">{t('likes.myLikes')}</h1>
            <p className="text-gray-600">{t('likes.viewProfiles')}</p>
          </div>
          
          <PremiumGate
            feature="Beğendiklerim Listesi"
            requiredTier="silver"
          >
            <LikesContent />
          </PremiumGate>
